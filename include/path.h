...
#define TraverseForAll {true, true, true, true}
	...
	// 回调函数版本
#ifndef PATH_CALLBACK
	#define PATH_CALLBACK std::function<void(TraverseData data)>
#endif
	bool _traverseC(const std::string &path, const PATH_CALLBACK &fileFunc, const PATH_CALLBACK &dirFunc, const std::string &selector, const TraverseMode &mode);
	bool _traverseFileC(const std::string &path, const PATH_CALLBACK &fileFunc, const PATH_CALLBACK &dirFunc, const std::string &selector, const TraverseMode &mode) {
		_finddata_t fileInfo;
		std::string currentPath = path + "\\" + selector;
		intptr_t handle = _findfirst(currentPath.c_str(), &fileInfo);
		if (handle == -1) return false;
	...
	bool _traverseDirC(const std::string &path, const PATH_CALLBACK &fileFunc, const PATH_CALLBACK &dirFunc, const std::string &selector, const TraverseMode &mode) {
		_finddata_t fileInfo;
		std::string currentPath = path + "\\*.*";
		intptr_t handle = _findfirst(currentPath.c_str(), &fileInfo);
		if (handle == -1) return false;
	...
	bool traverseC(std::string path, PATH_CALLBACK fileFunc = nullptr, PATH_CALLBACK dirFunc = nullptr, std::string selector = "*.*", TraverseMode mode = TraverseForAll) {
		pathDelete(path);
